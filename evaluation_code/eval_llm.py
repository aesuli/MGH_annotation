...
def get_gpt4_data(dataset_name, model_name, experiment):
    ...
    preds_ds = preds_ds.map(
        lambda x: {"id": x["custom_id"], "pred": split_at_regetso_gpt4(x["text_response"])})
    return preds_ds

def split_at_regetso_llama(x):
    for idx, i in enumerate(x.split()):
        if "regesto" in i.lower():
...
def eval_llm(args, model_name):
    ...
    count_per_instance = args.count_per_instance
    ...
    for dataset_name in dataset_names:
        ...
        for experiment in experiments:
            ...
            if count_per_instance:
                per_instance_metrics = evaluate.load("rouge")
                per_instance_ids = []
            for i in tqdm(preds_ds):
                id = i["id"]
                pred = i["pred"]
                ...
                if per_instance_metrics is None or per_instance_metrics.name != "rouge":
                    continue
                per_instance_metrics.add(reference=ref, prediction=pred)
                per_instance_ids.append(id)

            if per_instance_metrics is not None:
                results = per_instance_metrics.compute(use_aggregator=False)
                results = sorted(results["rouge2"])
                idxs = [h[0] for h in sorted([(i,j) for i,j in enumerate(results)], key=lambda x: x[1])]
                results_idxs = [per_instance_ids[i] for i in idxs]
                sorted_ref_df = ref_df.loc[results_idxs, :]
                sorted_pred_df = preds_ds.to_pandas().set_index("id").loc[results_idxs, :]
                sorted_ref_df["regesto_sintetico"] = sorted_pred_df["pred"]
                sorted_ref_df["score"] = results
                sorted_ref_df.reset_index().loc[
                    :, ["id", "apparato", "regesto", "regesto_sintetico", "testo esteso", "score"]
                ].to_excel(f"annotation_output/{model_name}_{dataset_name}_{experiment}.xlsx")

    out_df = convert_metrics_to_df(metrics, model_name)
    return out_df

def get_examples_df(per_instance_metrics, ref_df, preds_df):
    for dataset_name, experiment_metrics in per_instance_metrics.items():
        for experiment, _metrics in experiment_metrics.items():
            results = sorted(_metrics["results"], key=lambda x: x["value"]["rouge2"])
